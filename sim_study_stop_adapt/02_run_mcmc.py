...
mcmc = SingleBlockAMGS(log_posterior, start, iterations=500_000, stop_adapting=100_000)
mcmc_out = mcmc.run()
end_time = datetime.now()
print("MCMC took {} seconds".format(end_time - start_time))

# SAVE RESULTS
np.save(os.path.join(INFERENCE_DIR, f"posterior_{SIM_NUM}.npy"), mcmc_out)

# ESS
ess_out = effective_sample_size(mcmc_out[300_000:])
with open(os.path.join(INFERENCE_DIR, f"ESS_{SIM_NUM}.npy"), 'w') as ess_file:
